...
def main(
    ...
    if sdnq.common.use_torch_compile:
        ...
        sdnq_fp16_int7_tflops = benchmark_linear("SDNQ FP16 INT7", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="int7", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        sdnq_fp16_int6_tflops = benchmark_linear("SDNQ FP16 INT6", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="int6", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        sdnq_fp16_int5_tflops = benchmark_linear("SDNQ FP16 INT5", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="int5", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        sdnq_fp16_uint4_tflops = benchmark_linear("SDNQ FP16 UINT4", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint4", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        sdnq_fp16_uint3_tflops = benchmark_linear("SDNQ FP16 UINT3", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint3", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        sdnq_fp16_uint2_tflops = benchmark_linear("SDNQ FP16 UINT2", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint2", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        sdnq_fp16_uint1_tflops = benchmark_linear("SDNQ FP16 UINT1", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint1", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True), x, steps)
        ...
        sdnq_fp16_int7_svd_tflops = benchmark_linear("SDNQ FP16 INT7 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="int7", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
        sdnq_fp16_int6_svd_tflops = benchmark_linear("SDNQ FP16 INT6 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="int6", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
        sdnq_fp16_int5_svd_tflops = benchmark_linear("SDNQ FP16 INT5 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="int5", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
        sdnq_fp16_uint4_svd_tflops = benchmark_linear("SDNQ FP16 UINT4 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint4", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
        sdnq_fp16_uint3_svd_tflops = benchmark_linear("SDNQ FP16 UINT3 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint3", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
        sdnq_fp16_uint2_svd_tflops = benchmark_linear("SDNQ FP16 UINT2 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint2", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
        sdnq_fp16_uint1_svd_tflops = benchmark_linear("SDNQ FP16 UINT1 SVD", sdnq_quantize_layer(torch.nn.Linear(k,n, bias=True).to(device, dtype=dtype), weights_dtype="uint1", quantized_matmul_dtype="float16", torch_dtype=dtype, use_quantized_matmul=True, use_svd=True), x, steps)
    else:
