...
def get_forward_func(param_weights_dtype: str, quantized_matmul_dtype: str, use_grad_ckpt: bool, use_quantized_matmul: bool, use_static_quantization: bool, current_group_size: int) -> Callable:
    can_use_static_matmul = bool(
        ...
        and (
            ...
            or (param_weights_dtype in {"fp16", "float16"} and quantized_matmul_dtype in {"fp16", "float16"})
            or (param_weights_dtype in {"fp8", "float8_e4m3fn", "float8_e5m2"} and quantized_matmul_dtype in {"fp8", "float8_e4m3fn", "fp16", "float16"})
            )
        )
            ...
            if dtype_dict[quantized_matmul_dtype]["is_unsigned"]:
                if use_grad_ckpt:
                    if can_use_static_matmul:
                        from .layers.linear.linear_uint8.linear_uint8 import quantized_linear_forward_uint8_matmul
                        quantized_forward = quantized_linear_forward_uint8_matmul
                    else:
                        from .layers.linear.linear_uint8.linear_uint8_dynamic import quantized_linear_forward_uint8_matmul_dynamic
                        quantized_forward = quantized_linear_forward_uint8_matmul_dynamic
                else:
                    if can_use_static_matmul:
                        from .layers.linear.linear_uint8.linear_uint8_ckpt import quantized_linear_forward_uint8_matmul_ckpt
                        quantized_forward = quantized_linear_forward_uint8_matmul_ckpt
                    else:
                        from .layers.linear.linear_uint8.linear_uint8_dynamic_ckpt import quantized_linear_forward_uint8_matmul_dynamic_ckpt
                        quantized_forward = quantized_linear_forward_uint8_matmul_dynamic_ckpt
            else:
                if use_grad_ckpt:
                    if can_use_static_matmul:
