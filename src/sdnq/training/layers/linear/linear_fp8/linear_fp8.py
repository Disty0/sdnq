...
def quantize_fp_mm_input(input: torch.FloatTensor, dim: int = -1, do_input_reshape: bool = True, use_sr: bool = False, matmul_dtype: str = "float8_e4m3fn") -> tuple[torch.Tensor, torch.FloatTensor]:
    if do_input_reshape:
        input = input.flatten(0,-2)
    input, input_scale = quantize_fp_mm(input.to(dtype=torch.float32), dim=dim, matmul_dtype=matmul_dtype, use_sr=use_sr)
...
def fp8_matmul(
    ...
    input, weight, input_scale, scale, bias, bias_to_add_after, return_dtype, output_shape = get_fp8_matmul_inputs(
        input=input,
        weight=weight,
        scale=scale,
        bias=bias,
