...
def get_fp16_matmul_backward_inputs(input: torch.FloatTensor, hadamard: torch.FloatTensor | None) -> torch.FloatTensor:
...
class FP16MatmulBackwardCKPT(torch.autograd.Function):
    ...
    def forward(ctx, input: torch.FloatTensor, weight: SDNQTensor, bias: torch.FloatTensor | None = None) -> torch.FloatTensor:
        ...
        new_input, input_scale = get_fp16_matmul_backward_inputs(input, hadamard)
...
get_fp16_matmul_backward_inputs = compile_func(get_fp16_matmul_backward_inputs)
