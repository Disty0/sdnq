...
from ..linear_fp8.linear_fp8_dynamic import quantize_fp_mm_matmul


def get_fp16_matmul_dynamic_inputs(
...
def fp16_matmul_dynamic(
    ...
    input, weight, input_scale, scale, bias, bias_to_add_after, return_dtype, output_shape = get_fp16_matmul_dynamic_inputs(
        input=input,
        weight=weight,
        bias=bias,
        svd_up=svd_up,
    ...
    result = fp_scaled_mm_func(input, weight, input_scale, scale, bias=bias, out_dtype=return_dtype).view(output_shape)
...
if not include_mm_kernel_in_compile:
    get_fp16_matmul_dynamic_inputs = compile_func(get_fp16_matmul_dynamic_inputs)
else:
    fp16_matmul_dynamic = compile_func(fp16_matmul_dynamic)
